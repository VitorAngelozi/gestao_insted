...
def homepage(request):
    ...
    salas = Sala.objects.select_related(
        "curso", "andar", "curso__semestre_periodo"
    ).filter(curso__semestre_periodo__isnull=False)
    ...
    if semestre_id:
        ...
        semestre_ativo = (
            SemestrePeriodo.objects.filter(ativo=True)
    ...
    # Aplicar filtro por metodologia
    if metodologia:
        salas = salas.filter(metodologia=metodologia)
    ...
    for sala in salas_list:
        if sala.andar:  # Só processa se tiver andar
            ...
            if andar_id not in andares_dict:
                andares_dict[andar_id] = {"andar": sala.andar, "salas": []}
            andares_dict[andar_id]["salas"].append(sala)

    # Converter para lista e ordenar por número do andar
    andares_com_salas = sorted(
        andares_dict.values(), key=lambda x: x["andar"].numero if x["andar"] else 999
    )
    ...
    if semestre_selecionado:
        cursos_filtrados = Curso.objects.filter(
            semestre_periodo=semestre_selecionado
        ).order_by("nome")
    else:
        cursos_filtrados = Curso.objects.filter(
            semestre_periodo__isnull=False
        ).order_by("nome")
...
def exportar_excel(request):
    ...
    for sala in salas_list:
        ws.cell(row=row_num, column=1).value = (
            sala.andar.numero if sala.andar else "N/A"
        )
        ...
        ws.cell(row=row_num, column=9).value = sala.get_metodologia_display()
        ws.cell(row=row_num, column=10).value = (
            "Disponível" if sala.tem_lugares_livres() else "Lotado"
        )
    ...
    column_widths = [12, 20, 30, 15, 10, 10, 10, 12, 20, 15]
